...
class _QrisOutletScreenState extends State<QrisOutletScreen> {
  ...
  // Sumber QRIS outlet lain milik penjual ini yang SUDAH punya foto (untuk opsi "pakai dari
  // outlet lain" -- alih-alih upload ulang gambar yang sama persis). outletCode -> url.
  final Map<String, String> _qrisByOutlet = {};
  ...
  Future<void> _loadOutlets() async {
    ...
    // Cek sekaligus outlet mana saja yang sudah punya foto QRIS, untuk opsi "pakai dari
    // outlet lain". Jumlah outlet per penjual biasanya kecil, aman dipanggil paralel.
    await Future.wait(_outlets.map((o) async {
      final url = await _coreApi.getOutletQrisPhotoUrl(o.outlet_code);
      if (url != null) _qrisByOutlet[o.outlet_code] = url;
    }));
  ...
  Future<void> _loadPhoto(String code) async {
    ...
    if (mounted) {
      setState(() {
        ...
        _copyMode = false;
        _copyFrom = null;
        if (url != null) _qrisByOutlet[code] = url;
      });
    }
  }

  List<OutletResponse> get _copySources => _outlets
      .where((o) => o.outlet_code != _outletCode && _qrisByOutlet.containsKey(o.outlet_code))
      .toList();

  Future<void> _saveCopy() async {
    if (_copyFrom == null || _outletCode == null) return;
    setState(() => _copying = true);
    try {
      final resp = await _coreApi.copyOutletQrisPhoto(_outletCode!, _copyFrom!);
      if (resp.statusCode == 200) {
        if (mounted) Toast.success(context, 'Foto QRIS disalin dari outlet lain');
        await _loadPhoto(_outletCode!);
      } else {
        if (mounted) Toast.error(context, 'Gagal menyalin foto QRIS');
      }
    } catch (_) {
      if (mounted) Toast.error(context, 'Gagal menyalin foto QRIS');
    } finally {
  ...
  Future<void> _confirmDelete() async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        setState(() {
          _currentUrl = null;
          _qrisByOutlet.remove(_outletCode);
        });
      } else {
                ...
                : SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        SectionCard(
                          title: _currentUrl != null ? 'Ganti foto QRIS' : 'Simpan foto QRIS',
                          icon: Icons.qr_code_2,
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.stretch,
                            children: [
                              if (_copySources.isNotEmpty)
                                Padding(
                                        ...
                                        child: OutlinedButton(
                                          ...
                                          style: OutlinedButton.styleFrom(
                                              backgroundColor: _copyMode
                                                  ? AppColors.blueLight
                                                  : null),
                                          child: const Text('Pakai dari outlet lain'),
                                        ),
                                      ),
                                    ],
                                  ),
                              ...
                              if (_copyMode) ...[
                                ...
                                const SizedBox(height: 12),
                                ElevatedButton(
                                  onPressed: (_copyFrom == null || _copying)
                                      ? null
                                      : _saveCopy,
                                  child: Text(
                                      _copying ? 'Menyimpan...' : 'Pakai QRIS ini'),
                                ),
                              ] else ...[
                                if (_newFile != null)
                                  Padding(
                                ...
                                ElevatedButton(
                                  onPressed:
                                      (_newFile == null || _saving) ? null : _save,
