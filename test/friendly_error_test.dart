...
void main() {
  ...
  test('masalah server saat login bukan "salah password"', () {
    expect(f('[Service][LoginUser]: dial tcp 10.0.0.1:6379: connection refused'), msgServer);
    expect(f('[Service][LoginUser]: redis: connection pool timeout'), msgServer);
  });
  test('pesan bisnis & kode dibiarkan', () {
    ...
    expect(f('Stok tidak cukup untuk item ini'), 'Stok tidak cukup untuk item ini');
  });
  test('detail teknis disamarkan', () {
    ...
    expect(f("Key: 'X.Name' Error:Field validation for 'Name' failed"), msgServer);
    expect(f('Backup gagal: [Service][Backup]: Error 1045 access denied'), msgServer);
    expect(f('Network Error'), msgNetwork);
    expect(f('ClientException: Failed host lookup: x'), msgNetwork);
    expect(f(''), msgServer);
  });
  test('staging apa adanya', () {
    expect(friendlyErrorMessage('[Service][LoginUser]: Password did not match', production: false),
